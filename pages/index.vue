<template>
  <div>
    <div class="d-flex align-center justify-center header-shadow">
      <v-img class="my-4" max-height="50" max-width="125" src="/logo-color.svg">
      </v-img>
    </div>
    <v-container>
      <v-row class="my-4">
...
export default {
  ...
  async fetch() {
    this.events = await fetch(
      'https://api-squad5.herokuapp.com/events?_limit=12'
    ).then((res) => res.json())
...
<style scoped>
.header-shadow {
  box-shadow: 0px 2px 4px rgba(80, 71, 94, 0.12);
}
</style>
